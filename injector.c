...
static PyObject *_inject(PyObject *self, PyObject *args)
{
    ...
    // Using secure version to convert the string
    if (!PyArg_ParseTuple(args, "s", &dllPathStr))
    {
        Py_RETURN_NONE;
    }

    // mbstowcs_s returns an error code if conversion fails
    errno_t err = mbstowcs_s(&convertedChars, dllPath, MAX_PATH, dllPathStr, _TRUNCATE);
    if (err != 0)
    {
